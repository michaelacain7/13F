...
def search_sec_companies(query):
    """Search SEC EDGAR for companies/filers by name. Returns list of {cik, name, has_13f}."""
    results = []
    seen_ciks = set()
    query_lower = query.lower().strip()

    # ── Method 1: EDGAR EFTS full-text search (searches entity names in filings) ──
    try:
        efts_url = (
            f"https://efts.sec.gov/LATEST/search-index?"
            f"q=%22{requests.utils.quote(query)}%22&"
            f"forms=13F-HR,13F-HR%2FA&dateRange=custom&from=0&size=40"
        )
            ...
            for hit in data.get("hits", {}).get("hits", []):
                ...
                if ciks and entity_name:
                    ...
                    if cik not in seen_ciks:
                        seen_ciks.add(cik)
                        results.append({"cik": cik, "name": entity_name, "has_13f": True})
    except Exception as e:
        log.warning(f"EFTS search failed: {e}")

    # ── Method 2: EDGAR company search (Atom XML — much easier to parse than HTML) ──
    for form_type in ["13F-HR", ""] if not results else ["13F-HR"]:
        try:
            browse_url = (
                f"https://www.sec.gov/cgi-bin/browse-edgar?"
                f"company={requests.utils.quote(query)}&CIK=&type={form_type}&dateb=&"
                f"owner=include&count=40&search_text=&action=getcompany&output=atom"
            )
            ...
            if r.status_code == 200:
                ...
                for entry in root.iter('entry'):
                    # Extract CIK from <cik> tag or from <link> href
                    ...
                    if not cik:
                        ...
                        if link_el is not None:
                            ...
                            m = re.search(r'CIK=0*(\d+)', href, re.IGNORECASE)
                            if m:
                    ...
                    if not cik:
                        ...
                        if content_el is not None:
                            text = ET.tostring(content_el, encoding='unicode')
                            m = re.search(r'CIK[=:]?\s*0*(\d{5,10})', text)
                            if m:
                                cik = m.group(1).lstrip("0") or "0"

                    # Extract name from <title> or <company-name>
                    name = ""
                    for tag in ['company-name', 'title']:
                        name_el = entry.find(f'.//{tag}')
                        if name_el is not None and name_el.text:
                            name = name_el.text.strip()
                            # Clean up title format: "0000789019 - MICROSOFT CORP (13F-HR)"
                    ...
                    if cik and name and cik not in seen_ciks:
                        seen_ciks.add(cik)
                        has_13f = form_type == "13F-HR"
                        results.append({"cik": cik, "name": name, "has_13f": has_13f})

        except Exception as e:
            log.warning(f"Browse-edgar Atom search failed: {e}")

    # ── Method 3: Direct submissions API lookup (if query looks like a known name) ──
    # Try fetching company_tickers.json and searching it
    if len(results) < 3:
        try:
            r = requests.get(
            ...
            if r.status_code == 200:
                ...
                for key, entry in tickers_data.items():
                    ...
                    # Match on company name or ticker
                    if (query_lower in title_lower or
                        query_lower == ticker_lower or
                        title_lower.startswith(query_lower)):
                        seen_ciks.add(cik)
                        # Check if this entity has 13F filings
                        results.append({
                            "cik": cik,
                            "name": f"{title} ({ticker})",
                            "has_13f": None,  # Unknown, will verify if added
                        })
                        if len(results) >= 20:
                            break
        except Exception as e:
            log.warning(f"Company tickers search failed: {e}")

    # ── Method 4: For any results with unknown 13F status, quick-check via submissions ──
    for r_item in results:
        if r_item["has_13f"] is None:
            try:
    ...
    # ── Sort: 13F filers first, then by name relevance ──
    def sort_key(r):
        name_lower = r["name"].lower()
        exact = 0 if query_lower == name_lower else 1
        starts = 0 if name_lower.startswith(query_lower) else 1
        contains = 0 if query_lower in name_lower else 1
        has_13f = 0 if r["has_13f"] else 1
        return (has_13f, exact, starts, contains, name_lower)
            ...
            for block in blocks:
                ...
                if cusip:
                    ...
                    holdings[key] = {
                        ...
                        "value_thousands": int(value_m.group(1)) if value_m else 0,
