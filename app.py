...
<body>
    <div class="container">
        ...
        <div class="stats-row">
            ...
            <div class="stat-card"><div class="stat-label">Alerts Sent</div><div class="stat-value" id="statAlerts">0</div></div>
        </div>
...
async function fetchStatus(){try{const r=await fetch('/api/status'),d=await r.json();document.getElementById('clock').textContent=d.current_time_est+' | '+d.current_day;document.getElementById('statFilers').textContent=d.filer_count;document.getElementById('statChecks').textContent=d.checks_today;if(d.last_check)document.getElementById('statLastCheck').textContent=new Date(d.last_check).toLocaleTimeString();const b=document.getElementById('statusBadge'),s=document.getElementById('statusText');if(d.market_hours&&d.filer_count>0){b.className='status-badge active';s.textContent='SCANNING'}else if(!d.market_hours){b.className='status-badge inactive';s.textContent='AFTER HOURS'}else{b.className='status-badge inactive';s.textContent='IDLE'}}catch(e){}}
