...
# Suppress noisy APScheduler job execution logs
...
POLL_INTERVAL_SECONDS = 5  # Polling interval - checks 3 filers per tick via rotation
    ...
    try:
        ...
        for cik, data in monitored_filers.items():
            filers_save[cik] = {
                ...
                "holdings_current": data.get("holdings_current", {}),
                "holdings_prev": data.get("holdings_prev", {}),
            }
...
def load_state():
    ...
    _state_loaded = True
    try:
        # Diagnostic: check if persist directory exists and is writable
        log.info(f"💾 State persistence: dir={PERSIST_DIR}, file={PERSIST_FILE}")
        log.info(f"💾 Dir exists: {os.path.exists(PERSIST_DIR)}, File exists: {os.path.exists(PERSIST_FILE)}")
        if os.path.exists(PERSIST_DIR):
            try:
                contents = os.listdir(PERSIST_DIR)
                log.info(f"💾 Dir contents: {contents[:10]}")
            except Exception as e:
                log.warning(f"💾 Cannot list dir: {e}")

        if not os.path.exists(PERSIST_FILE):
            log.info("No saved state found, starting fresh.")
            log.info("⚠️  To persist state across deploys, set up a Railway Volume mounted at /data")
            return False
        ...
        for cik, data in saved_filers.items():
            if cik not in monitored_filers:
                has_holdings = bool(data.get("holdings_current"))
                has_filing = bool(data.get("last_filing_accession"))
                # Only needs init if we have NO data at all
                needs_init = not has_holdings and not has_filing
        ...
        log.info(f"State loaded: {len(saved_filers)} filers ({loaded_ready} ready, {loaded_needs_init} need init), {len(saved_alerts)} alerts (saved {state.get('saved_at', '?')})")
        return True
    except Exception as e:
...
DEFAULT_FILERS = [
    ...
    ("1364742", "JANA PARTNERS LLC"),                  # Barry Rosenstein (old entity)
    ("1998597", "JANA Partners Management, LP"),       # Barry Rosenstein (new entity)
    ("102909",  "VALUE ACT CAPITAL"),                  # Jeff Ubben / Mason Morfit
    ("1179392", "TWO SIGMA INVESTMENTS LP"),           # John Overdeck & David Siegel
    ...
    # Corporate 13F Filers
    ("1018724", "AMAZON COM INC"),                     # Amazon
    ("1045810", "NVIDIA CORP"),                        # NVIDIA
    ("1652044", "Alphabet Inc."),                      # Alphabet/Google
    ("1108524", "Salesforce, Inc."),                   # Salesforce
    ("50863",   "INTEL CORP"),                         # Intel
    ("858877",  "CISCO SYSTEMS, INC."),                # Cisco
    ("1321655", "Palantir Technologies Inc."),         # Palantir
]

def sec_request(url, timeout=15, max_retries=3):
    """Make a request to SEC with automatic retry on rate limiting."""
    for attempt in range(max_retries):
        try:
            r = requests.get(url, headers=SEC_HEADERS, timeout=timeout)
            if r.status_code == 429 or r.status_code == 403:
                wait = (attempt + 1) * 2  # 2s, 4s, 6s backoff
                log.warning(f"SEC rate limited ({r.status_code}) on {url}, waiting {wait}s (attempt {attempt+1}/{max_retries})")
                time.sleep(wait)
                continue
            r.raise_for_status()
            return r
            ...
            time.sleep(1)
            gc.collect()  # Free memory after cycle
        except requests.exceptions.ConnectionError:
            log.warning(f"Connection error on {url} (attempt {attempt+1}/{max_retries})")
            time.sleep(2)
...
def parse_13f_holdings(xml_url):
    try:
        r = sec_request(xml_url, timeout=20)
        raw_content = r.text
...
_no_13f_warned = set()  # Track filers already warned about no 13F filings
...
def check_filer(cik, filer_data):
    try:
        ...
        # Compare new filing against the MOST RECENT known holdings (holdings_current)
        # NOT holdings_prev (which is the quarter BEFORE the most recent)
        prev_holdings = filer_data.get("holdings_current", {})

        if not prev_holdings and len(filings_13f) > 1:
            log.info(f"No stored holdings for {filer_data['name']}, fetching previous filing for comparison")
        ...
        log.info(f"Comparing {filer_data['name']}: {len(curr_holdings)} current vs {len(prev_holdings)} previous holdings")
        ...
        filer_data["holdings_prev"] = filer_data.get("holdings_current", {})  # shift: old current becomes prev
        filer_data["holdings_current"] = curr_holdings  # new filing becomes current
...
def should_poll():
    ...
    # Monitor weekdays 6am-6pm ET (market hours)
    if now.weekday() >= 5:
...
def check_edgar_feed():
    """Check EDGAR recent filings RSS feed for new 13F filings.
    Returns True if feed was successfully checked."""
    global _poll_count
    _poll_count += 1

    # Only check EDGAR feed every 10th poll (~30 seconds) to avoid rate limits
    # Rotation handles the fast per-filer checks
    if _poll_count % 10 != 0:
        return True
    ...
    try:
        ...
        if _poll_count % 20 == 0:
            log.info(f"📡 Feed scan #{_poll_count}: {len(hits)} 13F filings today, monitoring {len(monitored_filers)} filers")
        ...
        if not hits:
            ...
            if _poll_count % 20 == 0:
                log.info(f"📡 Feed returned {total} total, {len(hits)} hits. Keys: {list(data.keys())[:5]}")
            return True

        # Build set of monitored CIKs
...
def check_edgar_feed_13d():
    ...
    Returns True if feed was successfully checked."""
    # Only check every 10th poll (~30 seconds) to reduce SEC load
    if _poll_count % 10 != 5:  # Offset from 13F feed check
        return True
    today = datetime.now(EST).strftime("%Y-%m-%d")
    url = (
        f"https://efts.sec.gov/LATEST/search-index?"
...
def check_next_filer_rotation():
    """PRIMARY detection method: check multiple filers per tick via round-robin.
    With 38 filers and 3 per tick at 3s interval = full cycle every ~40 seconds."""
    ...
    for _ in range(FILERS_PER_TICK):
        _filer_rotation_idx = _filer_rotation_idx % len(ciks)

        # Log when starting a new full rotation cycle
        if _filer_rotation_idx == 0:
            log.info(f"🔄 Starting new rotation cycle through {len(ciks)} filers")
        ...
        if not filer.get("_needs_init"):
            try:
                ...
                log.error(f"Rotation check error for {filer.get('name', cik)}: {e}")
            time.sleep(0.2)  # Small delay between checks to avoid rate limits
...
def monitor_loop():
    ...
    # PRIMARY: Rotation through filers — checks 3 filers per tick via SEC submissions API
    # Full cycle through all filers every ~40 seconds. Guaranteed reliable.
    check_next_filer_rotation()

    # SECONDARY: EDGAR feed — single request for all today's 13F filings (every 30s)
    # Bonus coverage but may be unreliable
    check_edgar_feed()
...
def load_default_filers():
    ...
    default_ciks = {cik for cik, _ in DEFAULT_FILERS}
    ...
    for cik, name in DEFAULT_FILERS:
        if cik not in monitored_filers:
            ...
            # Filer already exists (from state) — ensure it's marked as default
            monitored_filers[cik]["is_default"] = True

    # Count how many actually need init
    needs_init = sum(1 for f in monitored_filers.values() if f.get("_needs_init"))
    log.info(f"Seeded {len(DEFAULT_FILERS)} default filers. {needs_init} need initialization. Starting background data load...")

    # Step 2: Background thread to fetch actual filing data from SEC
    def _bg_load():
      try:
        # Build list of ALL filers that need init
        all_to_init = []
        seen = set()
        for cik, name in DEFAULT_FILERS:
            if cik not in seen:
                all_to_init.append((cik, name))
                seen.add(cik)
        # Add any non-default filers that also need init
        for cik, filer in list(monitored_filers.items()):
            if cik not in seen and filer.get("_needs_init"):
        ...
        for cik, name in all_to_init:
            ...
            _init_single_filer(cik)
            loaded_count += 1

            # SEC rate limit: pause between filers, longer pause every 5
            if loaded_count % 5 == 0:
                time.sleep(3)
            else:
                time.sleep(1)

        log.info(f"Background loading complete. Loaded {loaded_count} filers.")
        save_state()
      except Exception as e:
        log.error(f"CRITICAL: Background loading thread crashed: {e}")
        # Clear _needs_init for ALL filers to prevent permanent stuck state
        for filer in monitored_filers.values():
    ...
    <div class="container">
        ...
        <div class="add-section">
            ...
            <div class="input-row">
                ...
                <button class="btn btn-secondary" style="background:#f59e0b22;color:#f59e0b;border-color:#f59e0b44" onclick="retryStuck()">Retry Stuck</button>
            </div>
...
async function retryStuck(){showToast('Retrying stuck filers...');try{const r=await fetch('/api/retry-stuck',{method:'POST'}),d=await r.json();if(d.success){showToast(d.message);setTimeout(fetchFilers,3000);setTimeout(fetchFilers,8000);setTimeout(fetchFilers,15000);setTimeout(fetchFilers,25000)}}catch(e){showToast('Error','error')}}
...
def _init_single_filer(cik):
    """Initialize a single filer's holdings data from SEC. Thread-safe.
    If the latest filing is recent (within filing window), sends a Discord alert."""
    if cik not in monitored_filers:
        return False
    filer = monitored_filers[cik]
    try:
        info = get_company_info(cik)
        if not info:
            log.warning(f"Could not fetch info for {filer['name']} (CIK: {cik})")
            filer.pop("_needs_init", None)
            return False
        ...
        if filings_13f:
            ...
            # Parse latest holdings
            index_html, base_url = fetch_filing_index(cik, latest["accession"])
            infotable_url = find_infotable_url(index_html, base_url, cik, latest["accession"])
            curr_holdings = {}
            if infotable_url:
                curr_holdings = parse_13f_holdings(infotable_url)
            filer["holdings_current"] = curr_holdings
            filer["total_holdings"] = len(curr_holdings)

            # Parse previous holdings for comparison
            prev_holdings = {}
            if len(filings_13f) > 1:
                time.sleep(0.3)
                prev = filings_13f[1]
            ...
            # Check if this filing is RECENT (filed within last 7 days)
            # If so, send an alert — this handles fresh starts during filing season
            try:
                filing_date = datetime.strptime(latest["date"], "%Y-%m-%d").date()
                days_old = (datetime.now(EST).date() - filing_date).days
                if days_old <= 7 and curr_holdings:
                    changes = compare_holdings(prev_holdings, curr_holdings)
                    has_changes = (changes["new"] or changes["increased"] or
                                   changes["decreased"] or changes["closed"])
                    if has_changes:
                        log.info(f"📢 Init alert for {filer['name']}: filing from {latest['date']} "
                                 f"({len(curr_holdings)} current vs {len(prev_holdings)} previous)")
                        send_discord_alert(filer["name"], cik, latest["date"], changes,
                                           accession=latest["accession"])
                        filer["last_changes"] = changes
                        ...
                        log.info(f"Init {filer['name']}: filing from {latest['date']}, no changes vs previous quarter")
            except Exception as alert_err:
                log.warning(f"Init alert check failed for {filer['name']}: {alert_err}")

        else:
            log.warning(f"No 13F filings found for {filer['name']} (CIK: {cik})")

        filer.pop("_needs_init", None)
        log.info(f"Initialized {filer['name']} - {filer['total_holdings']} holdings, last filed {filer['last_filing_date']}")
        save_state()
        return True
    except Exception as e:
        log.error(f"Error initializing filer {filer['name']}: {e}")
        filer.pop("_needs_init", None)
        return False
...
def api_force_check(cik):
    ...
    # If filer is stuck on loading, initialize it first
    if filer.get("_needs_init"):
        success = _init_single_filer(cik)
        if success:
            return jsonify({"success": True, "message": f"Initialized {filer['name']} - {filer['total_holdings']} holdings"})
        else:
            return jsonify({"error": f"Failed to initialize {filer['name']}"}), 500

    # Force check: always do a FRESH comparison between latest 2 filings from SEC
    try:
        info = get_company_info(cik)
        if not info:
            return jsonify({"error": f"Could not fetch SEC data for {filer['name']}"}), 500

        filings_13f = find_13f_filings(info["filings"])
        if not filings_13f:
            return jsonify({"error": f"No 13F filings found for {filer['name']}"}), 404

        latest = filings_13f[0]
        index_html, base_url = fetch_filing_index(cik, latest["accession"])
        infotable_url = find_infotable_url(index_html, base_url, cik, latest["accession"])
        if not infotable_url:
            return jsonify({"error": f"Could not find infotable for latest filing"}), 500

        curr_holdings = parse_13f_holdings(infotable_url)
        if not curr_holdings:
            return jsonify({"error": f"Could not parse holdings from latest filing"}), 500
        ...
        changes = compare_holdings(prev_holdings, curr_holdings)
        log.info(f"Force check {filer['name']}: {len(curr_holdings)} current vs {len(prev_holdings)} previous holdings")
        send_discord_alert(filer["name"], cik, latest["date"], changes, accession=latest["accession"])
        ...
        summary = (f"{len(changes['new'])} new, {len(changes['increased'])} added, "
                   f"{len(changes['decreased'])} reduced, {len(changes['closed'])} closed")
        return jsonify({"success": True, "message": f"Force check {filer['name']}: {summary}"})

    except Exception as e:
        log.error(f"Force check error for {filer['name']}: {e}")
        return jsonify({"error": str(e)}), 500


@app.route("/api/retry-stuck", methods=["POST"])
def api_retry_stuck():
    """Retry all stuck/loading filers in a background thread."""
    stuck = [(cik, f["name"]) for cik, f in monitored_filers.items() if f.get("_needs_init")]
    if not stuck:
        return jsonify({"success": True, "message": "No stuck filers found"})

    def _retry():
        for i, (cik, name) in enumerate(stuck):
            log.info(f"Retrying stuck filer {name} ({i+1}/{len(stuck)})")
            _init_single_filer(cik)
            time.sleep(1.5)
        save_state()
        log.info(f"Retry complete for {len(stuck)} stuck filers")

    threading.Thread(target=_retry, daemon=True).start()
    return jsonify({"success": True, "message": f"Retrying {len(stuck)} stuck filers in background"})


@app.route("/api/debug")
def api_debug():
    """Debug endpoint showing filer states."""
    filer_states = []
    for cik, f in monitored_filers.items():
        filer_states.append({
            "cik": cik,
            "name": f.get("name", "?"),
            "needs_init": f.get("_needs_init", False),
            "is_default": f.get("is_default", False),
            "user_added": f.get("user_added", False),
            "has_holdings": bool(f.get("holdings_current")),
            "total_holdings": f.get("total_holdings", 0),
            "last_filing_date": f.get("last_filing_date"),
            "last_filing_accession": f.get("last_filing_accession"),
        })
    return jsonify({
        "total_filers": len(monitored_filers),
        "stuck": sum(1 for f in filer_states if f["needs_init"]),
        "ready": sum(1 for f in filer_states if not f["needs_init"]),
        "filers": sorted(filer_states, key=lambda x: (not x["needs_init"], x["name"])),
    })
