...
def search_sec_companies(query):
    ...
    # Method 1: EDGAR full-text search filtered to 13F-HR filers
    try:
        efts_url = (
    ...
    # Method 2: EDGAR company search (browse-edgar) - catches entities EFTS might miss
    try:
        browse_url = (
            f"https://www.sec.gov/cgi-bin/browse-edgar?"
            f"company={requests.utils.quote(query)}&CIK=&type=13F-HR&dateb=&"
            f"owner=include&count=20&search_text=&action=getcompany"
        )
        r = requests.get(browse_url, headers=SEC_HEADERS, timeout=15)
        if r.status_code == 200:
            # Parse HTML for company rows: CIK and company name
            rows = re.findall(
                r'CIK=(\d+)[^>]*>.*?<td[^>]*>\s*([^<]+?)\s*</td>',
                r.text, re.DOTALL
            )
            ...
            for cik_raw, name_raw in rows:
                cik = cik_raw.lstrip("0") or "0"
                name = name_raw.strip()
                if cik not in seen_ciks and name:
                    seen_ciks.add(cik)
                    results.append({"cik": cik, "name": name, "has_13f": True})
    except Exception as e:
        log.warning(f"Browse-edgar search failed: {e}")

    # Method 3: If no 13F-specific results, try general company search
    if not results:
        try:
            browse_url = (
                f"https://www.sec.gov/cgi-bin/browse-edgar?"
                f"company={requests.utils.quote(query)}&CIK=&type=&dateb=&"
                f"owner=include&count=20&search_text=&action=getcompany"
            )
            r = requests.get(browse_url, headers=SEC_HEADERS, timeout=15)
            if r.status_code == 200:
                ...
                for cik_raw, name_raw in rows:
                    ...
                    if cik not in seen_ciks and name:
                        seen_ciks.add(cik)
                        results.append({"cik": cik, "name": name, "has_13f": False})
        except Exception as e:
            log.warning(f"General company search failed: {e}")

    # Sort: 13F filers first, then by name relevance (exact match first)
