...
def check_edgar_feed():
    ...
    try:
        ...
        # Diagnostic: log response structure on first poll
        if _poll_count <= 3:
            top_keys = list(data.keys())[:5]
            sample_hit = None
            if hits:
                sample_hit = {k: str(v)[:80] for k, v in hits[0].get("_source", {}).items()}
            log.info(f"🔍 Feed diagnostic #{_poll_count}: url={url}")
            log.info(f"🔍 Response keys: {top_keys}, total hits: {len(hits)}")
            if sample_hit:
                log.info(f"🔍 Sample hit fields: {sample_hit}")
            elif not hits and data:
                # Maybe the structure is different - log what we got
                log.info(f"🔍 No hits.hits found. Raw keys: {list(data.keys())}")
                for k in list(data.keys())[:3]:
                    val = data[k]
                    if isinstance(val, dict):
                        log.info(f"🔍 data['{k}'] keys: {list(val.keys())[:5]}")

        # Log status every ~60 seconds (every 20 polls at 3s interval)
        if _poll_count % 20 == 0:
...
def check_next_filer_rotation():
    """Check ONE filer per tick via round-robin. Full cycle every ~2 minutes."""
...
def monitor_loop():
    ...
    # 1. EDGAR feed: single request catches all new 13F filings (fast but may miss)
    # 2. Rotation: directly check one filer per cycle (slower but guaranteed reliable)
    check_edgar_feed()
