...
function showSearchResults(results){const c=document.getElementById('searchResults');if(!results||!results.length){c.style.display='none';return}let h='<div style="font-size:12px;color:var(--text-muted);padding:8px 12px;border-bottom:1px solid var(--border)">'+results.length+' matches found — click to add:</div>';results.forEach(r=>{const badge=r.has_13f?'<span style="background:var(--accent-dim);color:var(--accent);padding:2px 6px;border-radius:3px;font-size:10px;margin-left:8px">13F FILER</span>':'<span style="background:var(--orange-dim);color:var(--orange);padding:2px 6px;border-radius:3px;font-size:10px;margin-left:8px">NO 13F</span>';if(r.has_13f){h+='<div class="search-result-item" onclick="pickSearchResult(\\x27'+r.cik+'\\x27)" style="padding:10px 16px;cursor:pointer;border-bottom:1px solid var(--border);transition:background 0.15s"><div style="display:flex;align-items:center;justify-content:space-between"><div><span style="font-weight:600;font-size:14px">'+esc(r.name)+'</span>'+badge+'</div><span style="font-family:JetBrains Mono,monospace;font-size:11px;color:var(--text-muted)">CIK '+r.cik+'</span></div></div>'}else{h+='<div style="padding:10px 16px;border-bottom:1px solid var(--border);opacity:0.45;cursor:not-allowed" title="This entity does not file 13F forms"><div style="display:flex;align-items:center;justify-content:space-between"><div><span style="font-weight:600;font-size:14px">'+esc(r.name)+'</span>'+badge+'</div><span style="font-family:JetBrains Mono,monospace;font-size:11px;color:var(--text-muted)">CIK '+r.cik+'</span></div></div>'}});c.innerHTML=h;c.style.display='block'}
...
def api_add_filer():
    ...
    # Warn if this entity doesn't file 13F forms
    if not filings_13f:
        return jsonify({
            "error": f"{info['name']} does not appear to file 13F forms. "
                     f"Only institutional investment managers with $100M+ AUM file 13F. "
                     f"Try searching for the fund/manager name instead."
        }), 400
